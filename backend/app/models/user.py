...
class User(Base, UUIDPrimaryKey, Timestamps):
    ...
    @property
    def min_possible_age(self) -> int:
        """Минимально возможный возраст: день рождения в этом году мог ещё
        не наступить, поэтому `age` завышен на единицу примерно у половины."""
        return self.age - 1
    ...
    def is_minor(self) -> bool:
        ...
        Считаем по МИНИМАЛЬНО возможному возрасту, а не по разнице годов.
        Иначе зарегистрировавшийся в 17 лет в декабре уже второго января
        числится совершеннолетним, хотя ему всё ещё 17, и система
        перестаёт требовать согласие родителя почти на год раньше срока.

        Цена ошибки несимметрична: лишнее согласие — неудобство,
        пропущенное — несовершеннолетний на охраняемой территории без
        разрешения законного представителя.
        """
        return self.min_possible_age < settings.ADULT_AGE
